...
export default function WaitlistItem({ data }: Props) {
  ...
  return (
    <Card className="w-3xl h-auto">
      <CardHeader>
        <h2 className="text-md flex items-center gap-2"><BellIcon size={16} /> { format(new Date(data.event?.opens_at), 'P p') }</h2>
        <CardTitle>
      ...
      <CardContent className="space-x-2">
        ...
        <Separator />
        <CardAction>
          {data.event?.status === 'open' && <Button className="cursor-pointer" disabled onClick={buyTickets}>Buy Tickets</Button>}
