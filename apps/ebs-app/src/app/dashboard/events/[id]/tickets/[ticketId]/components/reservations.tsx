...
type Props = {
  id?: number,
  reservations: Promise<{ data?: Reservation[], error?: string } | null>
}
export function TicketReservations({ reservations }: Props) {
  const response = use(reservations)
  return (
    <Card className="min-w-5xl">
      <CardHeader>
        <CardTitle>Reservations{response?.data?.length ? ` (${response?.data?.length})` : ''}</CardTitle>
      </CardHeader>
